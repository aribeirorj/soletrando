...
export function RankingScreen() {
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-6 px-4 py-10">
      <h2 className="text-xl font-bold">Ranking da turma</h2>
        ...
        {ranked.map((student, index) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="w-6 text-right font-bold text-brand-textMain">{index + 1}º</span>
              {index < 3 && <TrophyIcon className="text-brand-yellowDark" />}
              <span className="font-semibold">{student.name}</span>
            </div>
            <span className="text-sm text-muted-foreground">{student.totalScore} pontos</span>
          </li>
        ))}
      </ol>
